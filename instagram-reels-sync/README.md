...
4. 결과를 같은 스프레드시트의 **날짜 붙인 두 탭**에 저장합니다(기본값). 기존 탭은
   보존되어 실행 이력이 날짜별로 쌓입니다. 예: `릴스분석_2026-08-01`, `릴스요약_2026-08-01`.
   - **릴스분석_날짜**: 릴스 단위 상세 (조회수 / 평균 / 배수 / 좋아요 / 댓글 / 게시일 / 캡션),
     배수 내림차순 정렬, `배수 ≥ 기준`이면 `★` 표시
   - **릴스요약_날짜**: 계정 단위 요약 (릴스수 / 평균 / 최고 조회수 / 최고 릴스 URL / 상태)
   - `IG_DATED_TABS=0` 으로 두면 날짜 없이 고정 탭(`릴스분석`/`릴스요약`)에 덮어씁니다.
...
| `IG_OUTPUT_TAB` | `릴스분석` | 릴스 상세 결과 탭(기준 이름, 날짜가 뒤에 붙음) |
| `IG_SUMMARY_TAB` | `릴스요약` | 계정 요약 탭(기준 이름, 날짜가 뒤에 붙음) |
| `IG_DATED_TABS` | `1` | `1`=날짜 붙인 새 탭에 저장(기본), `0`=고정 탭에 덮어쓰기 |
