...
# 교안 품명 칸에 한국어 이름과 영어 이름을 나란히 적어 둔 제품이 있다.
# 화면에는 한국어 이름만 있으면 된다. 영어는 en 에 따로 있다.
영문꼬리 = re.compile(
    r"(?<=[가-힣0-9)\]])\s+(?:d['’]?Alba|[A-Z])[A-Za-z0-9'’&.\- ]{12,}$"
)


def 이름다듬기(ko):
    """한국어 이름 뒤에 그대로 붙어 있는 영어 이름을 뗀다."""
    return 영문꼬리.sub("", ko).strip()
